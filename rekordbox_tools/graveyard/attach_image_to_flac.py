...
def attach_picture_to_file(audio_file: str, picture: Picture):
...
def find_all_tracks_with_same_album(audio_file) -> List[str]:
...
def attach_existing_album_art_to_tracks(results: List):
    ...
    if len(valid_image_path_for_album) >= 1:
        ...
        raise Exception(f"Cannot find a valid path for image")
    for c in results:
        if c.ImagePath is None or c.ImagePath == "":
            # TODO: update with
            c.ImagePath = new_image_path
            print(f"commiting change to imagePath for {c.FileNameL} from None to {new_image_path}")
            db.commit()
        else:
            pass


def main(audio_file, image_file):
    # picture = create_picture(image_file)
    # attach_picture_to_file(audio_file, picture)
    # print(f"All done attaching cover image to {audio_file}")
